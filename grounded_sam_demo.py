...
#     print(f"Dino:Inference time: {dino_time:.6f} seconds")
...
#     print(f"SAM:Inference time: {sam_time:.6f} seconds")
...
#     output_filename = f"test_{output_filename}_output.jpg"
#     plt.axis('off')
#     plt.savefig(
...
def get_grounding_output(model, image, caption,
...
def show_mask(mask, ax):
    color = np.array([128/255, 64/255, 128/255, 0.8])
    h, w = mask.shape[-2:]
    mask_image = mask.reshape(h, w, 1) * color.reshape(1, 1, -1)
    ax.imshow(mask_image)
...
if __name__ == "__main__":
    ...
    print("Loading SAM...")
    sam = sam_model_registry[args.sam_version](
    ...
    if os.path.isdir(args.input_image):
        image_list = sorted(
            glob.glob(os.path.join(args.input_image, "*.jpg")) +
            glob.glob(os.path.join(args.input_image, "*.png")) +
            glob.glob(os.path.join(args.input_image, "*.jpeg"))
        )
    else:
        image_list = [args.input_image]
    ...
    for image_path in tqdm(image_list):
        ...
        boxes_cpu = boxes_filt.detach().cpu()
        transformed_boxes = predictor.transform.apply_boxes_torch(
            boxes_cpu, image_cv.shape[:2]
        ).to(device)
        ...
        print(f"{filename} | DINO: {dino_time:.3f}s | SAM: {sam_time:.3f}s")

    # ==========================
    # FPS
    # ==========================
    total_time = time.time() - total_start
    fps = len(image_list) / total_time
